...
const Player = () => {
  const [isVolumeController, setIsVolumeController] = useState(false);
  return (
    <div className="fixed bottom-0 right-0 left-0 bg-[#f4f4f4ff] flex flex-col">
      ...
      <div className="flex justify-between items-center mb-3 px-3">
        ...
        >
          <LuHardDriveDownload className="text-gray-700 cursor-pointer hover:text-gray-500 text-2xl lg:text-3xl lg:mr-2" />
          <HiSpeakerWave className="text-gray-700 cursor-pointer hover:text-gray-500 text-2xl lg:text-3xl hidden lg:block" onMouseEnter={() => setIsVolumeController(true) }onMouseEnter={() =>  setIsVolumeController(true) }
          onMouseOut={()=> setIsVolumeController(false)}/>
          <VolumeController isVolumeController={isVolumeController}
           />
        </div>
      </div>
    </div>
